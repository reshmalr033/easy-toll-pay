    ...
    <table class="table table-responsive"style="background: #E5E5E5">
        <?php
                            ...
                            if(isset($_POST['b1']))
                            {
                                ...
                                $qry="select amount from account_bank where account_no='$_POST[t1]'";
                                $res=mysql_query($qry);
                                $amt=mysql_result($res,0);
                                
                                $tot=$amt+$_POST['t2'];
                                
                                
                              $upd=mysql_query("update account_bank set amount= '$tot'where account_no='$_POST[t1]'");
                           if(mysql_affected_rows()>0)
                           {
                                $date=date('Y-m-d H:i:s');
                                 mysql_query("insert into cash_deposit(amount_from,amount_to,amount,date,status)values('self','$_POST[t1]','$_POST[t2]','$date','1')");
                                if($upd>0)
                           {
                                 if(mysql_affected_rows()>0)
